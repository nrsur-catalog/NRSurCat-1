...
def generate_fake_posterior(npts=100):
    ...
    prior = bilby.gw.prior.BBHPriorDict(dict(
        mass_ratio=TG(name='mass_ratio', minimum=0.125, maximum=1, mu=inj['mass_ratio'], sigma=0.1),
        chirp_mass=TG(name='chirp_mass', minimum=25, maximum=100, mu=inj['chirp_mass'], sigma=10),
        luminosity_distance=TG(name='luminosity_distance', minimum=1e2, maximum=5e3, mu=inj['luminosity_distance'],
                               sigma=1e2),
        dec=TG(name='dec', minimum=-np.pi / 2, maximum=np.pi / 2, mu=inj['dec'], sigma=np.pi / 4),
        ra=TG(name='ra', minimum=0, maximum=2 * np.pi, boundary='periodic', mu=inj['ra'], sigma=np.pi / 4),
        theta_jn=TG(name='theta_jn', minimum=0, maximum=np.pi, mu=inj['theta_jn'], sigma=np.pi / 4),
        psi=TG(name='psi', minimum=0, maximum=np.pi, boundary='periodic', mu=inj['psi'], sigma=np.pi / 4),
        phase=TG(name='phase', minimum=0, maximum=2 * np.pi, boundary='periodic', mu=inj['phase'], sigma=np.pi / 4),
        a_1=TG(name='a_1', minimum=0, maximum=0.99, mu=inj['a_1'], sigma=0.1),
        a_2=TG(name='a_2', minimum=0, maximum=0.99, mu=inj['a_2'], sigma=0.1),
        tilt_1=TG(name='tilt_1', minimum=0, maximum=np.pi, mu=inj['tilt_1'], sigma=np.pi / 4),
        tilt_2=TG(name='tilt_2', minimum=0, maximum=np.pi, mu=inj['tilt_2'], sigma=np.pi / 4),
        phi_12=TG(name='phi_12', minimum=0, maximum=2 * np.pi, boundary='periodic', mu=inj['phi_12'],
                  sigma=np.pi / 4),
        phi_jl=TG(name='phi_jl', minimum=0, maximum=2 * np.pi, boundary='periodic', mu=inj['phi_jl'],
                  sigma=np.pi / 4),
    ))
    return prior.sample(npts)


def generate_fake_result(n=100, outdir="outdir", event_name="test", fname=None):
